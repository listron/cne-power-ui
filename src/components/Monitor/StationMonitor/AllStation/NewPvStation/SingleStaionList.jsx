    ...
    return (
      <div className={`${styles[getStatusName[`${currentStatus}`]]} ${styles.staionCard}  ${alarm && styles.alarm}`}
        ...
        <div className={styles.bottom}>
          ...
          <Link to={`/monitor/singleStation/${singleStation.stationCode}?showPart=${invertType}`}
                className={styles.dataColumn}>
            低效逆变器 <span
            className={styles[`${singleStation.lowEfficiencyInverterNum > 0 ? 'red' : 'grey'}`]}> {dataFormats(singleStation.lowEfficiencyInverterNum, '--', 0)}</span>
          </Link>
