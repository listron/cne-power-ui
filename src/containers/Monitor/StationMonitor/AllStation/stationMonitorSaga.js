...
function* getMonitorStation(action) {//获取所有/风/光电站信息
  ...
  try {
    ...
    const nameArr = ['windMonitorStation', 'pvMonitorStation', 'allMonitorStation'][payload.stationType];
    payload[nameArr] = {};
    yield put({
      type: allStationAction.changeMonitorstationStore,
      payload,
...
function* monthplanpower(action) { // 多电站月累计与计划发电量图(光伏)
  ...
  const url = `${baseurl + Path.APISubPaths.monitor.getMonthPalnPower}${startDate}/${endDate}/${nowDate}/${regionName}`;
...
function* getSingleCharts(action) { // 五分钟获取每一个电站的出力图
  ...
  const endTime = moment().endOf('day').utc().format();
  const url = `${baseurl}${Path.APISubPaths.monitor.getPvCapabilitydiagrams}`;
  const stationCodes = yield select(state => { // 获取现在所有的电站 即使现在新增加的也可以
