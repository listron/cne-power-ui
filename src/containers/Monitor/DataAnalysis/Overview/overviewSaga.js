...
function *getOverviewStation({ payload }){ // 电站相关数据信息
  try {
    ...
    //get /api/v3/dataoverriew/devicetype/{stationCode}
    const url = `${APIBasePath}${monitor.getOverviewStation}/${stationCode}`;
    const response = yield call(request.get, url);
    if (response.code === '10000') {
      const stationData = response.data || {};
      yield call(easyPut, 'fetchSuccess', { stationData });
    } else { throw response; }
  } catch (error) {
    console.log(error);
...
function *getOverviewDates({ payload }){ // 电站各日完整率
  // get /api/v3/dataoverriew/station/{stationCode}/{deviceTypeCode}/{month}
  try {
    const url = `${APIBasePath}${monitor.getOverviewDates}`;
    const response = yield call(request.get, url);
    if (response.code === '10000') {
