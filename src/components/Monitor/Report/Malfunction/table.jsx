...
class TableList extends Component {
  ...
  componentDidMount() {
    const { getMalfunctionList, pageNum, pageSize } = this.props;
    getMalfunctionList({ stationCodes: '', pageNum, pageSize })
  }
  onPaginationChange = ({ pageSize, currentPage }) => { // 分页器操作
    const { getMalfunctionList } = this.props;
    getMalfunctionList({ pageNum: currentPage, pageSize })
    this.props.changeMalfunctionStore({ pageNum: currentPage, pageSize, })
  }
  ontableSort = (pagination, filter, sorter) => {
    const { getMalfunctionList, stationType, year, month, powerSelectYear, dateType, pageSize, pageNum } = this.props;
    ...
    getMalfunctionList(prams)
    this.props.changeMalfunctionStore({ sort, sortType })
  }
  ...
  render() {
    const { total, pageSize, pageNum, malfunctionList } = this.props;
    const columns = this.initMonthColumn();
    const dataSource = malfunctionList.map((e, i) => ({
      ...e, key: i,
      pr: `${e.pr ? e.pr : '--'}%`,
