...
function* getInspectDetail(action) { // 获取工单详情数据
  ...
  try {
    yield put({
      type: newInspectDetailAction.changeStore,
      payload: {
        loading: true,
      },
    });
...
export function* newWatchInspectDetail() {
  yield takeLatest(newInspectDetailAction.getInspectDetail, getInspectDetail);
  yield takeLatest(newInspectDetailAction.setInspectCheck, setInspectCheck);
}
