...
class WindStation extends Component {
  ...
  onHandleStation = (e) => {
    this.setState({
      singleDeviceType: e.target.value,
    });
  }

  onSelectedDeviceType = (e) => { // 选择设备
    this.props.changeSingleStationStore({ deviceTypeCode: +(e.target.value) });
  }
  ...
  render() {
    ...
    const alarmStatus = alarmList ? !(alarmList instanceof Array) && alarmList.deviceList && alarmList.deviceList.some(e => e.alarmNum > 0) || (alarmList.length > 0 && alarmList.some(e => e.warningStatus)) : false;
        ...
        <div className={styles.windContainer}>
          <div className={styles.windList}>
            <div className={styles.threadAndDevice} id="deviceType" >
              ...
              {deviceTypeCode === '101' &&
                <RadioGroup onChange={this.onHandleStation} value={singleDeviceType} className={styles.singleDeviceTypeBox}>
                  <RadioButton key={0} value={0} >全部</RadioButton>
                  {stautus.map(e => (<RadioButton key={e.deviceStatusCode} value={e.deviceStatusCode}> {e.text} {this.getStatusNum(e.deviceStatusCode)}</RadioButton>))}
                </RadioGroup>
              }
            </div>
            <div>
