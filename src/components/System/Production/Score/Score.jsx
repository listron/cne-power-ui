...
class ScoreMain extends Component {
    ...
    componentDidMount() {
        this.props.getScoreConfig({ reportType: "1" })
    }
    ...
    stationCheck = (e) => {// 电站切换
        ...
        if (edit) {
            ...
            this.props.getScoreConfig({ reportType: `${e.target.value}` })
        }
    }
    ...
    default = () => { // 恢复默认
        ...
        this.props.getScoreConfig({reportType})
    }
    ...
    confirmWarningTip = () => { // 确认跳转
        ...
        this.props.getScoreConfig({ reportType:stationTypes})
    }

    cancelWarningTip = () => { // 取消
        const { stationTypes } = this.state;
        this.setState({ showWarningTip: false, stationTypes: stationTypes === '2' ? "1" : "2" })
        this.props.getScoreConfig({ reportType: stationTypes === '2' ? "1" : "2"})
    }
    ...
    render() {
        ...
        return (
            <div className={styles.scoreBox}>
                ...
                {!!edit &&
                    <div className={styles.scoreLine}>
                        <div><span>电站初始分</span> <Input placeholder="请填写" onChange={this.initChange} value={basicScore} /> </div>
